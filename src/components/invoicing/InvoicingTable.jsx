...
import DocumentModal from "../../components/invoicing/DocumentModal"
...
export default function InvoicingTable({ setType, setDate, setOption }) {
  ...
  // Mapa igual que el usado en DropdownStatus
  const statusMap = {
        ...
        {isActive &&
          createPortal(
            ...
            >
              <li>
                <a
                  className="dropdown-item blanco-status"
                  style={{ fontSize: "80%" }}
                  href="#"
                  onClick={() => handleStatusChange("blanco-status")}
                >
                  Pendiente
                </a>
              </li>
              <li>
                <a
                  className="dropdown-item gris-status"
                  style={{ fontSize: "80%" }}
                  href="#"
                  onClick={() => handleStatusChange("gris-status")}
                >
                  Generada
                </a>
              </li>
              <li>
                <a
                  className="dropdown-item amarillo-status"
                  style={{ fontSize: "80%" }}
                  href="#"
                  onClick={() => handleStatusChange("amarillo-status")}
                >
                  Enviada
                </a>
              </li>
              <li>
                <a
                  className="dropdown-item rojo-status"
                  style={{ fontSize: "80%" }}
                  href="#"
                  onClick={() => handleStatusChange("rojo-status")}
                >
                  Rechazada
                </a>
              </li>
              <li>
                <a
                  className="dropdown-item verde-status"
                  style={{ fontSize: "80%" }}
                  href="#"
                  onClick={() => handleStatusChange("verde-status")}
                >
                  Aprobada
                </a>
              </li>
  ...
  // Estados de los filtros de color
  const [activeStatuses, setActiveStatuses] = useState([]);

  // Función para alternar estado activo/inactivo del filtro
  const toggleStatusFilter = (statusClass) => {
    setActiveStatuses((prev) => {
      if (prev.includes(statusClass)) {
        // si ya está activo, lo quitamos
        const newStatuses = prev.filter((s) => s !== statusClass);
        applyFilter(newStatuses);
        return newStatuses;
      } else {
        // si no está activo, lo agregamos
        const newStatuses = [...prev, statusClass];
        applyFilter(newStatuses);
        return newStatuses;
      }
    });
  ...
  // Aplica el filtrado de acuerdo a los botones activos
  const applyFilter = (statuses) => {
    if (statuses.length === 0) {
      // si no hay botones activos, mostramos todos los datos
      setFilteredData(data);
    } else {
      const allowedColors = statuses.map((s) => statusMap[s]);
      const filtered = data.filter((item) => {
        const ultimo = item.statusPrefacturas[item.statusPrefacturas.length - 1];
        return ultimo && allowedColors.includes(ultimo.color_status);
      });
          ...
          {showFilter && (
            <div className="d-flex align-items-center">
              ...
              <div className="btn-group btn-group-sm ms-2" role="group">
                {["blanco-status", "gris-status", "amarillo-status", "rojo-status", "verde-status"].map((statusClass) => (
                  <button
                    key={statusClass}
                    type="button"
                    className={`btn ${statusClass} ${activeStatuses.includes(statusClass) ? "active" : ""}`}
                    onClick={() => toggleStatusFilter(statusClass)}
                  >
                    P
                  </button>
                ))}
        ...
        {loading ? (
          ...
          <>
            <div className="d-flex justify-content-start align-items-center mb-2">
              ...
              >
                <i
                  className={`bi bi-chevron-up collapse-icon ${
                    isOpen ? "" : "rotate"
                  }`}
                ></i>
              </button>
                        ...
                        <tr>
                          ...
                          <td>
                            {item.statusPrefacturas.length > 0 && (
                              <DropdownStatus
                                ultimo={
                                  item.statusPrefacturas[
                                    item.statusPrefacturas.length - 1
                                  ]
                                }
                                item={item}
                              />
                          ...
                          <td className="text-end">
                            {formatValue(item.total_consumido)}
                          </td>
                          <td className="text-center">
                            {getComparacionIcon(
                              comparaciones[item.id_primer_nivel]
                            )}
                          </td>
                          <td>
                            <button
                              ...
                              data-bs-title="Comentario"
                              data-bs-placement="top"
                              data-bs-content={
                                item.cliente?.comentario || "Sin comentario"
                              }
                            >
                              <span
